...
Ideas that are new and interesting will be explored and those that are troubling we will question and add suguestion. 
...
The idea of betweenness is in it's simpiliest form when concerning points on a line, but this idea can be generalized to many other structures. 
Structures of points on a plane are mentioned, used connected paths rather than lines, and graphs concerning betweenness of vertices in relation to internal vertices. 
Betweenness is then laid out for sets of objects before reviewing the insertion principle. At this time it is relevant to list a few definitions we will continue to reference throughout this paper.

\begin{itemize}

      \item \underline{Point}: A point is a uniquely determined location in space quantifiable by one or more reference variables. 

      \item \underline{Vertex}: Vertices are the fundamental building blocks of graphs, they are thought of as a possible point accessible within a graph.

      \item \underline{Path}: A path is a mode of transportation between points, or vertices alike, paths can be defined as directional or undirectional, thought of one-way or two-way roads.

      \item \underline{Line}: A line is a specific type of path that has no curvature with respect to all of it's variables. Lines are characterized as being "straight."

      \item \underline{Edge}: An edge is simply a path between two vertices within a graph. For our discussion, edges do not have to be lines. 

      \item \underline{Multiple Edges}: A graph that is defined as having multiple edges is a graph that that has more than one edge connecting any two vertices.
      
      \item \underline{Set}: A set is a collection of objects. These objects can take many different forms; for instance: points, vertices, edges, etc. 
      ...
      \item \underline{Undirected Graph}: An undirected graph is a collection of edges and vertices, in which every edge can be traversed in either direction. 
      ...
      \item \underline{Internal Vertex}: An internal vertex in analogous to an internal point given the path is an edge between two vertices,  $ v_i $ and $ v_f $, with a vertex $ v $ in between. 
...
\noindent
To understand the insertion principle, we must first come to the realization that given any two unique points, there exists a third point that is between them. 
This has a great deal of extensions, and a very interesting one mentioned is that there is always a rational number between a rational and an irrational. 
Using the notation from the article, this is written as $ \Q | \Q | \R \backslash \Q $. 
In general the notation for an object $ B $ between $ A $ and $ C $ is given by, $ A | B | C $. 
...
\noindent
From this we can see that it is relatively obvious that when one point, $ I $, is between a point $ A $, and a point $ B $, that is in between $ A $, and $ C $, then $ I $ is also between the same two points, $ A $, and $ C $.
Figure 1 is available for a graphical representation of this principal. It is now relevant to realize that the notation used for betweenness exhibits a symmetry as $  (A | B | C) \equiv (C | B | A) $. 
...
It follows that this same principal holds for arbitrarily closed sets within a topological space. 
Here $ A , B , C $ are either vertices or closed sets. The authors outline the proof and follow with a formalization of syntax and semantics. 
An axiomatic approach is taken within this encapusalting proof, Thus the authors next outline the axioms needed. 
Lastly each axiom implies finite graphs, know as soundness, and then the converse such that any finite graph implies each axiom, know as completeness. 
...
Figure 2 and Figure 3 provide a drawn representation in terms of vertices and closed sets. 
...
The trivial path axiom simply states that there is nothing in between two sets, given they share an object. 
...
The shortest path axiom states that the quickest route to a set is from the boundary of the first, given there is a set in between the two. 
This makes sense as you are virtually starting from a point in between the two sets. 
...
Transitivity follows as if there is no path from two different sets to an element, such that the path passes some other in between set, then there should also be no path between the two different sets such that it must past through some in between set.
...
\begin{proof}[\textbf{Proof}]
      ...
      \begin{itemize}
            ...
            \item $ i $ is not between $ a $ \& $ c $, therefore the index $ i $ is some number such that $ 0 \nleq i \nleq f $, or $ f < i < 0 $.
      ...
      Again referencing the insertion principal, we now take the second assumption that $ A | I \cup C | B_1 $.
      It should now be obvious that we have reached a contradiction. 
      $ B_1 $ is between $ A $ \& $ C $, therefore $ C $ is not between $ A $ \& $ B_1 $ which implies $ A | I | B_1 $. 
...
Overall this was a very interesting paper that I will be saving to read again at a later date. 
...
\begin{figure}
      ...
      \centering
      \includegraphics[width=1\textwidth]{Insertion.png}
      \caption{\label{Insertion.png}
...
\begin{figure}
      ...
      \centering
      \includegraphics[width=1\textwidth]{Vert-Insert.png}
      \caption{\label{Vert-Insert.png}
      Illustrated here is a graphical representation of the strong insertion principal with respect to vertices.}
...
\begin{figure}
      ...
      \centering
      \includegraphics[width=1\textwidth]{Set-Insert.png}
      \caption{\label{Set-Insert.png}
...
\begin{figure}
      ...
      \centering
      \includegraphics[width=1\textwidth]{Collorary.png}
      \caption{\label{Collorary.png}
...
\begin{figure}
      ...
      \centering
      \includegraphics[width=1\textwidth]{Trivial.png}
      \caption{\label{Trivial.png}
...
\begin{figure}
      ...
      \centering
      \includegraphics[width=1\textwidth]{Axioms.png}
      \caption{\label{Axioms.png}
