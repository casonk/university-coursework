...
\begin{proof}
  ...
  as we only perform rational operations and the square roots of the coefficients must exist to have pur centers and radai. 
...
\begin{proof}
  ...
  But if the coefficients on $ \sqrt{c} $ is non zero, then it is implied that $ \sqrt{c} \in F_{k} $, which is a contradiction.
  Thus the coefficient on $ \sqrt{c} $ must be zero, and $ 2 = a^{3} + 3ab^{2}c $ alone.
