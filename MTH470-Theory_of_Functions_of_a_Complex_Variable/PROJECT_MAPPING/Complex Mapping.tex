...
\title{$ \C $omplex Geometry \& $ \C $onformal Mapping}
...
A brief look at curve parameterization will be used as it will be imporntant with respect to conformal maps. 
...
In this context, and continuing throughout this paper, $ i $ denotes the imaginary number, where $ i = \sqrt{-1} $ and $ i^2 = -1 $.
...
\item Lastly, we can define the polar representation of $ z $ in terms of its norm, and $\vartheta$, referred to as its argument,  $ z \coloneqq  |z|e^{i\vartheta} $. This is derived from the trigonometric definitions for $ x $ and $ y $.  
...
Conversely the region is considered open given the elements are not allowed on the curve. 
In specific, $ Z $ is considered open given each element $ z \in Z $ can be surrounded by an open disc of radius $ r \gneq  0 $.
Elements of $ Z $ and $ W $ can then lie inside/outside the curve, or possibly in between some 2 curves  $ \gamma_{1},\gamma_{2} $ or $ \Gamma_{1},\Gamma_{2} $
We can then parameterize these curves in terms of $ t $ where $ \gamma(t), \Gamma(t) $ trace out the respective curves given $ t_{i} \leq t \leq t_{f} $
...
$ k $ is referred to as the winding number. There is no limit to the number of curves that define a region, or that lie in a region. 
Figures 3 and 4 take a moment to show this visually. 
\\
We will take a brief detour to now define a few imporntant terms. 
...
such that no $ \zeta $ falls on a parametric curve $ \gamma(t) $ used to define $ Z $, hence $ Z $ must be an open region.
Keep in mind that this image $ \omega $ is not the same as the image $ w $, in fact because $ \omega $ is the image of the derivative, it is best thought of a tangent vector. 
...
Adversly, in the equivalent definition, we see that the derivative is defined as the difference of the imaginary contribution, $ Im(z) = y $, of $ \nu $ and $ \mu $.
...
What is particularlly interesting here is that we can see a prevalent scalling from $ |\vartriangle x| $ to $ |\vartriangle f_{1}| $ and from $ |\vartriangle y| $ to $ |\vartriangle f_{2}| $,
...
At this point it is relevant to note that when multiplying two complex numbers, the mechanics are such that the norms are multiplied but the arguments are added. 
...
At this point it is appropriate to introduce the explicit formula for a $ m\ddot{o}bius $ transformation.
...
This verbiage stems from the fact that the numerator, $ az + b $, and the denomenator, $ cz + d $, are both of linear form.
...
This equation thus implies that $ f' \neq 0 $ as $ \xi = f^{-1}(z) $ is to be differentiable. We now have the full criteria for conformal maps. 
...
point in the direction the paths are moving when they cross $ z $. 
...
This does indeed happen in a few situations. $ \mathcal{J}_f $ could be the identity matrix, or some scalar multiple of it, as well as a rotational matrix. \dots
...
In computer science conformal geometry is quite crucial for image manipulation and distortion, while preserving angles. 
...
\begin{figure}
      ...
      \centering
      \includegraphics[width=1\textwidth]{Rectangular Map.png}
      \caption{\label{Rectangular Map.png}
...
\begin{figure}
      ...
      \centering
      \includegraphics[width=1\textwidth]{Polar Coordinates.png}
      \caption{\label{Polar Coordinates.png}
...
\begin{figure}
      ...
      \centering
      \includegraphics[width=1\textwidth]{Regions.png}
      \caption{\label{Regions.png}
      Illustrated here are closed regions where a point $ z $ may lie on the parametric curve $ \gamma $, 
      and open regions where $ z $ can always be surrounded by an open disc, as it cannot lie on $ \gamma $.}
...
\begin{figure}
      ...
      \centering
      \includegraphics[width=1\textwidth]{Parameterization.png}
      \caption{\label{Parameterization.png}
...
\begin{figure}
      ...
      \centering
      \includegraphics[width=.72\textwidth]{CRG.png}
      \caption{\label{CRG.png}
      ...
      These are the changes in the real and imaginary input respective point $ z_{1} $ and $ z_{2} $ are chosen to specifically move along these axis. 
...
\begin{figure}
      ...
      \centering
      \includegraphics[width=1\textwidth]{Mobius.png}
      \caption{\label{Mobius.png}
...
\begin{figure}
      ...
      \centering
      \includegraphics[width=1\textwidth]{Reimann.png}
      \caption{\label{Reimann.png}
...
\begin{figure}
      ...
      \centering
      \includegraphics[width=1\textwidth]{Angles.png}
      \caption{\label{Angles.png}
...
\begin{figure}
      ...
      \centering
      \includegraphics[width=1\textwidth]{Escher.jpg}
      \caption{\label{Escher.jpg}
