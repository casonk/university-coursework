...
We want to use the $ \chi^{2} $ test to decide whether there is sufficient evidence at level of significance $ \alpha = 0.05 $ 
...
\noindent
We want to test for a (un)fair die, and thus that all sides have equal probability, are hypotheses are thus \dots \\
...
Since you will be estimating a parameter, you will subtract one additional degree to
...
\noindent
We first need the $ MLE $ for a random variable distributed on $ Exponential(\lambda) $. \\
